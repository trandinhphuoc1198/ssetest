...
def app(environ, start_response):
    ...
    if path == "/step1":
        ...
        img = qrcode.make('https://192.168.1.2/step2?id=yourkaiinID')
        img.save('yourkaiinID.png')
        with open('yourkaiinID.png','rb') as f:
            headers.append(("Content-Length",f"{os.path.getsize('./yourkaiinID.png')}"))
            start_response('200 OK',headers)
            yield f.read()
    elif path == "/step2":
        if environ['REQUEST_METHOD'] == 'POST':
            with open('yourkaiinID.jpg','wb') as ff:
...
def step3(environ, start_response):
    ...
    with open('./yourkaiinID.jpg','rb') as f:
        data = f.read()
